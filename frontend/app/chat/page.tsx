...
const WS_URL = process.env.NEXT_PUBLIC_WS_URL || "wss://signal-xnx9.onrender.com";
...
export default function ChatPage() {
  ...
  useEffect(() => {
    if (!selectedConversation || currentUserId === null) return;

    const conversationId = selectedConversation.id;
    console.log("Connecting WebSocket:", conversationId, "user:", currentUserId);
    ...
    ws.onmessage = (event) => {
      ...
      try {
        ...
        if (data.type === "message") {
          const incomingMessage: Message = data.data;
          setMessages((previous) => {
            const exists = previous.some((msg) => msg.id === incomingMessage.id);
    ...
    return () => {
      console.log("Closing WebSocket:", conversationId);
      ws.close();
  ...
  const getConversationName = (conversation: Conversation) => {
    ...
    return otherMember?.display_name || otherMember?.username || "Unknown User";
  };
  ...
  const getOtherMember = (conversation: Conversation) => {
    return conversation.members.find((member) => member.user_id !== currentUserId);
  };
  ...
  const createDirectConversation = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        alert(data.detail || "User not found or already in conversation");
        return;
      }

      console.log("Direct conversation created:", data);
      ...
      <aside className="w-[360px] bg-white border-r border-gray-200 flex flex-col">
        ...
        <div className="px-5 py-5 border-b border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-gray-900">Signal</h1>
              <p className="text-xs text-gray-400 mt-1">@{currentUsername}</p>
            </div>
            <button
              onClick={logout}
        ...
        <div className="flex-1 overflow-y-auto">
          {filteredConversations.length === 0 ? (
            ...
            filteredConversations.map((conversation) => {
              ...
              const isOnline =
                conversation.type === "DIRECT" ? otherMember?.is_online : false;
        ...
        <div className="border-t border-gray-100 p-4 space-y-2">
          ...
          <button
            ...
            className="w-full rounded-xl bg-gray-200 text-gray-700 py-3 font-medium hover:bg-gray-300"
          >
            + New Group
          </button>
        </div>
        ...
        {!selectedConversation ? (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-gray-400 mt-2">Select a conversation to start messaging.</p>
            </div>
          </div>
        ) : (
          <>
              ...
              <div className="ml-3">
                ...
                <p className="text-xs text-green-500">
                  {selectedConversation.type === "GROUP"
                    ...
                    : getOtherMember(selectedConversation)?.is_online
                    ? "online"
                    : "offline"}
                </p>
              </div>
              <div className="ml-auto flex gap-2">
                <button className="h-10 w-10 rounded-full hover:bg-gray-100">🔍</button>
                <button className="h-10 w-10 rounded-full hover:bg-gray-100">⋮</button>
              </div>
            </header>
            ...
            <div className="bg-blue-50 border-b border-blue-100 px-6 py-2 text-center">
              <span className="text-xs text-blue-600">
                🔐 Messages are simulated encrypted for this demo
              </span>
            </div>
            ...
            <div className="bg-white border-t border-gray-200 p-4">
              <div className="flex items-center gap-3">
                <button className="h-11 w-11 rounded-full hover:bg-gray-100 text-xl">+</button>
                <input
                  value={message}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl w-[420px] p-6">
            ...
            <div className="flex gap-3 mt-6">
              ...
              >
                {creatingDirect ? "Creating..." : "Start Chat"}
              </button>
            </div>
          </div>
        </div>
